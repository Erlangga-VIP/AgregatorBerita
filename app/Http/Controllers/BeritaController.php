...
public function sync()
{
    try{
        ...
        ->accept('application/rss+xml, application/xml, text/ml')
        ->get($this->feedUrl);

        if ($response->failed()){
            return redirect()
            ->route('berita.index')
            ->with('eror', 'Gagal mengambil data feed dari website SMKN 1 Bawang.');
        }
    ...
    $jumlahTersimpan++;
        ...
        return redirect()
        ->route('berita.index')
        ->with('succes', 'Sinkronisasi berhasil. Total data diproses: ' . $jumlahTersimpan . 'berita.');
    } catch (\Throwable $e) {
        return redirect()
        ->route('berita.index')
        ->with('eror', 'Terjadi kesalahan: ' . $e->getMessage());
    }
}
...
public function truncate()
{
    ...
    return redirect()
    ->route('berita.index')
    ->with('succes', 'Semua data berhasil dihapus.');
}
}
