...
use App\Models\Berita; use Illuminate\Console\Command; use Illuminate\Support\Facades\Http; 
...
{ 
    ...
    { 
        ...
        try { 
            ...
            if ($response->failed()) { 
                $this->error('Gagal mengambil feed.');                 return self::FAILURE; 
            } 
...
LIBXML_NOCDATA); 
            ...
            if (!$xml || !isset($xml->channel->item)) {                 $this->error('Format feed tidak valid.');                 return self::FAILURE; 
            } 
            ...
            foreach ($xml->channel->item as $item) { 
                ...
                if (isset($item->category)) { 
                    $kategoriData = []; 
                    foreach ($item->category as $cat) {                         $kategoriData[] = trim((string) $cat); 
                    } 
                ...
                if (isset($item->pubDate) && !empty((string) $item->pubDate)) { 
                    $timestamp = strtotime((string) $item->pubDate);                     if ($timestamp !== false) { 
            ...
            $this->info("Sinkronisasi selesai. Total data diproses: {$jumlah}");             return self::SUCCESS; 
        } catch (\Throwable $e) { 
            $this->error('Terjadi kesalahan: ' . $e->getMessage());             return self::FAILURE; 
        } 
    } 
} 
